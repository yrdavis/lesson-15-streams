...
//	/* Sort the exam results from lowest to highest. */
	@Test
	// Deleted cast of (double) in test, as it was causing an error
	// changed return type to List<Double>
...
//	/* Remove the duplicate spells from a list, and alphabetize them. */
	...
	public void testRemoveDuplicates() throws Exception {
		List<String> spellsCastThisWeek = List.of("expelliarmus", "riddikulus", "lumos", "obliviate", "lumos", "accio",
				"accio", "accio", "lumos", "alohomora", "lumos", "expelliarmus");

		List<String> spellsCastThisWeek2 = List.of("accio, alohomora, expelliarmus, lumos, obliviate, riddikulus");
		assertEquals(spellsCastThisWeek2.toString(),
				AlgorithmsUsingStreams.getDistinctSpellsInAlphabeticalOrder(spellsCastThisWeek).toString());
	}

//	/* Search the String array for instances of "bam" and "pow", then capitalize them. */
	@Test
	public void testMap() throws Exception {
		String[] batmanScript = "Bam. If you kill a killer, the number of killers in the room remains the same. Bam. Pow. I have one power. I never give up. Pow."
				.split("\\.");
		// Full stops replaced with double spaces, and a comma.
		// List<String> batmanList = Arrays.asList(batmanScript);
		// System.out.println(batmanList);
		assertEquals("[BAM, BAM, POW, POW]", AlgorithmsUsingStreams.getBatPhrases(batmanScript).toString());

	}

//	/* Count the number of o's in a String. */
	@Test
